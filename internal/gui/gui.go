...
func Run() {
	...
	// Auto-start the proxy on launch when "AutoStart" was enabled. Run it on
	// the main event loop (after the window is shown) so the status bar and
	// any error dialogs render correctly. This must happen BEFORE ShowAndRun,
	// which blocks until the window is closed.
	if cfg.Autostart {
		fyne.Do(startProxy)
	}
