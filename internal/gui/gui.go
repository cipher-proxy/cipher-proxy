...
func Run() {
	...
	logScrollScroller := container.NewBorder(nil, nil, nil, nil, logScroll)

	content := container.NewBorder(controls, statusBar, nil, nil, logScrollScroller)
	w.SetContent(content)
	w.Resize(fyne.NewSize(680, 980))
	w.ShowAndRun()
...
// configureAutostart installs or removes the GNOME autostart desktop entry so
